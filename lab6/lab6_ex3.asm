...
ADD R4, R4, #0 ;check if flag is set
BRz YES
BRp NO
...
LOOP_TO_UPPER
	LDR R2, R1, #0 ;load array into R2
	ADD R2, R2, #0 ;see if it reached the end
	BRz OKAY
	
	AND R2, R2, R4 ;AND array with x5F (ASCII conversion)
	STR R2, R1, #0 ;store the result in R1
